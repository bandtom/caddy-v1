...
function get_ip_location_cn($ip)
{
    if (function_exists('curl_init')) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://lab.helingqi.com/ipip/" . $ip);
        ...
        $result  = file_get_contents('http://lab.helingqi.com/ipip/' . $ip, false, stream_context_create($options));
        if ($result === false) {
            $result = [];
        }
    }
    $location = json_decode($result, 1);
    return $location['res']['address'];
}
...
echo $os[1];?>(<?php echo $server_addr; ?>)&nbsp;&nbsp;
...
function getSysinfo() {
    $.getJSON('?method=sysinfo', function (data) {
        ...
        for (var dev in netdev) {
            var info = netdev[dev],
                rx = data.netdev[dev].rx - info.rx,
                tx = data.netdev[dev].tx - info.tx
            $('#netdev_'+ dev + '_human_rx').html(data.netdev[dev].human_rx)
            $('#netdev_'+ dev + '_human_tx').html(data.netdev[dev].human_tx)
            $('#netdev_'+ dev + '_delta_rx').html((rx / 1024 > 1024) ? (rx / 1024 / 1024).toFixed(2) + 'M/s' : (rx / 1024).toFixed(2) + 'K/s')
            $('#netdev_'+ dev + '_delta_tx').html((tx / 1024 > 1024) ? (tx / 1024 / 1024).toFixed(2) + 'M/s' : (tx / 1024).toFixed(2) + 'K/s')
        }
        netdev = data.netdev
    });
}
